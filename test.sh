...
grep -q 'mise activate bash' summon.sh || fail "summon.sh must initialize mise in bashrc"
...
grep -q '.bun/bin' summon.sh || fail "summon.sh must add bun global bin to PATH"
...
grep -q '"npm:@openai/codex" = "0.130.0"' mise.toml || fail "mise.toml must manage codex"
